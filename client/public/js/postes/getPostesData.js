...
export async function getPostData(devis_id) {
  const token = localStorage.getItem('token');
  console.log('📤 Appel API avec devis_id =', devis_id);
  try {
    const response = await fetch(`http://localhost:3000/api/postes?devis_id=${devis_id}`, {
      method: 'GET',
      headers: {
